"""Collect Web3 news from public RSS feeds."""
...
class NewsCollectionError(RuntimeError):
    """Raised when Web3 news cannot be collected or saved."""
...
def collect_web3_news() -> dict[str, Any]:
    """Fetch configured free feeds and return normalized Web3 news."""
    ...
    if not unique_articles:
        ...
        raise NewsCollectionError(f"No Web3 news was collected. {details}".strip())
...
def save_web3_news(news_data: dict[str, Any]) -> Path:
    """Save Web3 news to a UTC date-stamped JSON file."""
    date_stamp = datetime.now(timezone.utc).strftime("%Y-%m-%d")
    output_path = OUTPUT_DIRECTORY / f"web3_news_{date_stamp}.json"
    try:
        ...
        raise NewsCollectionError(f"Unable to save Web3 news: {exc}") from exc
    return output_path


def main() -> int:
    """Collect Web3 news and save it locally."""
    try:
        output_path = save_web3_news(collect_web3_news())
    except NewsCollectionError as exc:
        print(f"Web3 news collection failed: {exc}", file=sys.stderr)
        return 1
    print(f"Web3 news saved to: {output_path}")
    return 0
